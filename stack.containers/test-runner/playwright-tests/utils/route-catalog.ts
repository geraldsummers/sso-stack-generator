...
export const browserRouteCatalog: BrowserRoute[] = [
  ...
  },
  {
    ...
    visual: {
      pathForUser: (user) => `/SOGo/so/${encodeURIComponent(user.username)}/Preferences#!/calendars`,
      fileStem: 'sogo-authenticated',
      matcher: /\bPreferences\b|\bCalendar\b|\bDefault reminder\b|\bWeek begins on\b|\bTime Zone\b/i,
      selector: 'text=/Preferences|Default reminder|Week begins on|Time Zone/i',
      loginLabel: 'Keycloak',
      oidcStartPath: '/SOGo/',
