...
class KeycloakIdentityConfigTest {
    ...
    fun `keycloak service is a default core identity service`() {
        ...
        assertFalse(caddyfile.contains("header X-Trusted-Proxy-Secret {\$WORKSPACE_PROXY_AUTH_SECRET}"))
        assertFalse(caddyfile.contains("remote_ip private_ranges"))
    ...
    fun `service routes enforce keycloak group rbac at the edge`() {
        ...
        assertTrue(caddyfile.contains("import keycloak_group_allow search admins|operators"))
