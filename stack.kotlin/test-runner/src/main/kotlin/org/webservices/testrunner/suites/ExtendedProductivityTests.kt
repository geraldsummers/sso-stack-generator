...
suspend fun TestRunner.extendedProductivityTests() = suite("Extended Productivity Tests") {
    ...
    test("qBittorrent: Web UI is accessible") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }

        val response = client.getRawResponse(endpoints.qbittorrent!!)
        val body = requireOkResponse(response, "qBittorrent web UI")
    ...
    test("qBittorrent: API version endpoint") {
        ...
        requireStatus(
            response,
            setOf(HttpStatusCode.OK, HttpStatusCode.Unauthorized, HttpStatusCode.Forbidden),
            "qBittorrent version endpoint returned unexpected status"
        )
        if (response.status == HttpStatusCode.OK) {
    ...
    test("qBittorrent: torrent API requires local session auth") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }
        ...
        requireStatus(
            response,
            setOf(HttpStatusCode.Unauthorized, HttpStatusCode.Forbidden),
            "qBittorrent torrents API should reject unauthenticated direct access"
        )
        println("      ✓ Torrent API requires local session auth")
    }

    test("qBittorrent: WebUI serves static assets") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }

        val response = client.getRawResponse("${endpoints.qbittorrent}/css/login.css")
        requireStatus(
            response,
            setOf(HttpStatusCode.OK, HttpStatusCode.NotModified),
            "qBittorrent static assets were not served"
        )
        println("      ✓ WebUI static assets served")
    }

    test("qBittorrent: Torrent management API requires local session auth") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }

        val response = client.getRawResponse("${endpoints.qbittorrent}/api/v2/torrents/info?limit=1")
        requireStatus(
            response,
            setOf(HttpStatusCode.Unauthorized, HttpStatusCode.Forbidden),
            "qBittorrent torrent management API should reject unauthenticated direct access"
        )
        println("      ✓ Torrent management API requires local session auth")
    }

    test("qBittorrent: RSS feed API requires local session auth") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }
        ...
        requireStatus(
            response,
            setOf(HttpStatusCode.Unauthorized, HttpStatusCode.Forbidden),
            "qBittorrent RSS API should reject unauthenticated direct access"
        )
        println("      ✓ RSS API requires local session auth")
    }

    test("qBittorrent: Download statistics endpoint requires local session auth") {
        require(endpoints.qbittorrent != null) { "qBittorrent endpoint not configured" }
        ...
        requireStatus(
            response,
            setOf(HttpStatusCode.Unauthorized, HttpStatusCode.Forbidden),
            "qBittorrent statistics API should reject unauthenticated direct access"
        )
        println("      ✓ Statistics API requires local session auth")
    }
